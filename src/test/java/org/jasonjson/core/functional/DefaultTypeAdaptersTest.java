 ...
 * Functional test for Json serialization and deserialization for common classes for which default
 support is provided in Jason. The tests for Map types are available in {@link MapTest}.
 *
...
public class DefaultTypeAdaptersTest extends TestCase {
  ...
  public void testClassSerialization() {
    ...
    gson = new JasonBuilder().registerTypeAdapter(Class.class, new MyClassTypeAdapter()).create();
  ...
  public void testDateSerializationWithPattern() throws Exception {
    ...
    Jason gson = new JasonBuilder().setDateFormat(DateFormat.FULL).setDateFormat(pattern).create();
    Date now = new Date(1315806903103L);
    String json = gson.toJson(now);
  ...
  public void testDateSerializationInCollection() throws Exception {
    ...
    Locale.setDefault(Locale.US);
    try {
      Jason gson = new JasonBuilder().setDateFormat("yyyy-MM-dd").create();
