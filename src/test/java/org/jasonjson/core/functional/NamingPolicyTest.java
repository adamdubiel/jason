...
import org.jasonjson.core.common.TestTypes.ClassWithSerializedNameFields;
import org.jasonjson.core.common.TestTypes.StringWrapper;
...
public class NamingPolicyTest extends TestCase {
  ...
  public void testGsonWithNonDefaultFieldNamingPolicySerialization() {
    Jason gson = builder.setFieldNamingPolicy(FieldNamingPolicy.UPPER_CAMEL_CASE).create();
  ...
  public void testGsonWithLowerCaseDashPolicySerialization() {
    Jason gson = builder.setFieldNamingPolicy(FieldNamingPolicy.LOWER_CASE_WITH_DASHES).create();
    StringWrapper target = new StringWrapper("blah");
  ...
  public void testGsonWithLowerCaseUnderscorePolicySerialization() {
    Jason gson = builder.setFieldNamingPolicy(FieldNamingPolicy.LOWER_CASE_WITH_UNDERSCORES)
        .create();
    StringWrapper target = new StringWrapper("blah");
  ...
  public void testGsonWithUpperCamelCaseSpacesPolicySerialiation() {
    Jason gson = builder.setFieldNamingPolicy(FieldNamingPolicy.UPPER_CAMEL_CASE_WITH_SPACES)
        .create();
    StringWrapper target = new StringWrapper("blah");
    assertEquals("{\"Some Constant String Instance Field\":\""
        + target.someConstantStringInstanceField + "\"}", gson.toJson(target));
  ...
  public void testDeprecatedNamingStrategy() throws Exception {
    Jason gson = builder.setFieldNamingStrategy(new UpperCaseNamingStrategy()).create();
    ClassWithDuplicateFields target = new ClassWithDuplicateFields(10);
  ...
  public void testAtSignInSerializedName() {
    assertEquals("{\"@foo\":\"bar\"}", new Jason().toJson(new AtName()));
  }
