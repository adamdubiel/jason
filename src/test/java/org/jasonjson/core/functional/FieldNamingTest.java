...
import static org.jasonjson.core.FieldNamingPolicy.IDENTITY;
import static org.jasonjson.core.FieldNamingPolicy.LOWER_CASE_WITH_DASHES;
import static org.jasonjson.core.FieldNamingPolicy.LOWER_CASE_WITH_UNDERSCORES;
import static org.jasonjson.core.FieldNamingPolicy.UPPER_CAMEL_CASE;
import static org.jasonjson.core.FieldNamingPolicy.UPPER_CAMEL_CASE_WITH_SPACES;
...
public final class FieldNamingTest extends TestCase {
  public void testIdentity() {
    Jason gson = new JasonBuilder().setFieldNamingPolicy(IDENTITY).create();
  ...
  public void testUpperCamelCase() {
    Jason gson = new JasonBuilder().setFieldNamingPolicy(UPPER_CAMEL_CASE).create();
    assertEquals("{'LowerCamel':1,'UpperCamel':2,'_LowerCamelLeadingUnderscore':3," +
        "'_UpperCamelLeadingUnderscore':4,'Lower_words':5,'UPPER_WORDS':6," +
        "'annotatedName':7}",
        gson.toJson(new TestNames()).replace('\"', '\''));
  ...
  public void testUpperCamelCaseWithSpaces() {
    Jason gson = new JasonBuilder().setFieldNamingPolicy(UPPER_CAMEL_CASE_WITH_SPACES).create();
  ...
  public void testLowerCaseWithUnderscores() {
    Jason gson = new JasonBuilder().setFieldNamingPolicy(LOWER_CASE_WITH_UNDERSCORES).create();
  ...
  public void testLowerCaseWithDashes() {
    Jason gson = new JasonBuilder().setFieldNamingPolicy(LOWER_CASE_WITH_DASHES).create();
    assertEquals("{'lower-camel':1,'upper-camel':2,'_lower-camel-leading-underscore':3," +
        "'_-upper-camel-leading-underscore':4,'lower_words':5,'u-p-p-e-r_-w-o-r-d-s':6," +
        "'annotatedName':7}",
        gson.toJson(new TestNames()).replace('\"', '\''));
